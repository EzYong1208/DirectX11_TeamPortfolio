#include "stdafx.h"
#include "POT_Object_Iruka.h"
#include "GameInstance.h"
#include "Level_Loading.h"
#include "Command_Manager.h"
#include "Attack_Mask_Range.h"

CPOT_Object_Iruka::CPOT_Object_Iruka(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CActor(pDevice, pDeviceContext)
{
}

CPOT_Object_Iruka::CPOT_Object_Iruka(const CPOT_Object_Iruka& rhs)
	: CActor(rhs)
{
}

HRESULT CPOT_Object_Iruka::NativeConstruct_Prototype()
{
...
HRESULT CPOT_Object_Iruka::NativeConstruct(void* pArg)
{

	if (pArg != nullptr)
	{

		POT_IRUKA_INIT_DESC temp;
		memcpy(&temp, pArg, sizeof(POT_IRUKA_INIT_DESC));
	...
	//빌보드
	XMStoreFloat3(&m_vExtraScale, ACT_POT_IRUKA_BILLBOARD_OFFSET_EXTRA_SCALE);
	XMStoreFloat3(&m_vLocalLocation, ACT_POT_IRUKA_BILLBOARD_OFFSET_LOCATION);
	XMStoreFloat3(&m_vExtraLocation, ACT_POT_IRUKA_BILLBOARD_OFFSET_EXTRA_LOCATION);
	XMStoreFloat3(&m_vLocalDegree, ACT_POT_IRUKA_BILLBOARD_OFFSET_ROTATION);
	XMStoreFloat3(&m_vExtraDegree, ACT_POT_IRUKA_BILLBOARD_OFFSET_EXTRA_ROTATION);
...
_int CPOT_Object_Iruka::Tick(_double TimeDelta)
{
	if (m_Dead == true)
		return 4;
...
_int CPOT_Object_Iruka::LateTick(_double TimeDelta)
{
...
HRESULT CPOT_Object_Iruka::Render()
{
	//1. 부모 객체 : 상수들 바인드
...
HRESULT CPOT_Object_Iruka::Render_Shadow()
{
	//1. 부모 객체 : 상수들 바인드
	if (FAILED(__super::Render()))
		return E_FAIL;
...
HRESULT CPOT_Object_Iruka::Render_OutLine()
{
	//1. 부모 객체 : 상수들 바인드
	if (FAILED(__super::Render()))
		return E_FAIL;
...
_int CPOT_Object_Iruka::Check_CombatState()
{
...
HRESULT CPOT_Object_Iruka::SetUp_Components()
{
...
HRESULT CPOT_Object_Iruka::SetUp_Colliders()
{
	//1. 붙힐뼈를 가진 모델
...
#pragma region COLLIDER_ATTACK
	...
	XMStoreFloat3(&ColliderDesc.vScale, ACT_POT_IRUKA_COLLIDER_ATTACK_01_Scale);
	XMStoreFloat4(&ColliderDesc.vTranslation, ACT_POT_IRUKA_COLLIDER_ATTACK_01_Translation);

	DEBUG_BREAK(m_pColliderSlotCom->Insert_Collider(
		CColliderSlot::COLLIDER_BODY_WEAPON,
		ACT_POT_IRUKA_COLLIDER_ATTACK_01_NAME,
		LEVEL_STATIC,
...
HRESULT CPOT_Object_Iruka::SetUp_Model()
{
...
void CPOT_Object_Iruka::Update_CollisionGroup()
{
	//시작시 좌측일때
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CPOT_Object_Iruka::Set_Pos(_vector Pos)
{
	m_pTransform->Set_State(CTransform::STATE_POSITION, Pos);
}
void CPOT_Object_Iruka::Set_Ratio(_double _Ratio)
{
	m_pTransform->Set_Ratio(_Ratio);
}
void CPOT_Object_Iruka::Set_ExHeight(_double _ExHegiht)
{
	m_pTransform->Set_ExtraHeight(_ExHegiht);

}
CPOT_Object_Iruka* CPOT_Object_Iruka::Create(ID3D11Device* pDevice,
	ID3D11DeviceContext* pDeviceContext)
{
	CPOT_Object_Iruka* pInstance = new CPOT_Object_Iruka(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CPOT_Object_Iruka");
...
CGameObject* CPOT_Object_Iruka::Clone(void* pArg)
{
	CPOT_Object_Iruka* pInstance = new CPOT_Object_Iruka(*this);
