#include "stdafx.h"
#include "Effect_BGT_LTL_RogerPuch.h"
#include "GameInstance.h"

CEffect_BGT_LTL_RogerPuch::CEffect_BGT_LTL_RogerPuch(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_LTL_RogerPuch::CEffect_BGT_LTL_RogerPuch(const CEffect_BGT_LTL_RogerPuch& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_LTL_RogerPuch::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_LTL_RogerPuch::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_LTL_RogerPuch::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_LTL_RogerPuch::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_LTL_RogerPuch::Render()
{
...
_uint CEffect_BGT_LTL_RogerPuch::SetMaterialIndex()
{
	_double fParentCurAcc = m_EffectDesc.m_pParent->Get_ModelSlot()->Get_PlayAcc(MODEL_TAG_DEFAULT_BODY);
	_uint	iResult = 0;

	if (m_iCount == 0 && fParentCurAcc >= 10.f)
	{
		++m_iCount;
		m_iMaterialIndex = 0;
	}
	if (m_iCount == 1 && fParentCurAcc >= 15.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 2 && fParentCurAcc >= 20.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 3 && fParentCurAcc >= 25.f)
	{
		++m_iCount;
		m_iMaterialIndex = 0;
	}
	if (m_iCount == 4 && fParentCurAcc >= 30.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 5 && fParentCurAcc >= 35.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 6 && fParentCurAcc >= 40.f)
	{
		++m_iCount;
		m_iMaterialIndex = 0;
	}
	if (m_iCount == 7 && fParentCurAcc >= 45.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 8 && fParentCurAcc >= 50.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 9 && fParentCurAcc >= 55.f)
	{
		++m_iCount;
		iResult = ISDEAD;
	}
...
CEffect_BGT_LTL_RogerPuch* CEffect_BGT_LTL_RogerPuch::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_LTL_RogerPuch* pInstance = new CEffect_BGT_LTL_RogerPuch(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_LTL_RogerPuch");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_LTL_RogerPuch::Clone(void* pArg)
{
	CEffect_BGT_LTL_RogerPuch* pInstance = new CEffect_BGT_LTL_RogerPuch(*this);
...
void CEffect_BGT_LTL_RogerPuch::Free()
{
	__super::Free();
}
