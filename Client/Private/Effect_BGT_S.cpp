#include "stdafx.h"
#include "Effect_BGT_S.h"
#include "GameInstance.h"

CEffect_BGT_S::CEffect_BGT_S(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_S::CEffect_BGT_S(const CEffect_BGT_S& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_S::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_S::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_S::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_S::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_S::SetMaterialIndex()
{
...
CEffect_BGT_S* CEffect_BGT_S::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_S* pInstance = new CEffect_BGT_S(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_S");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_S::Clone(void* pArg)
{
	CEffect_BGT_S* pInstance = new CEffect_BGT_S(*this);
