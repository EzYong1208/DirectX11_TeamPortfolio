#include "stdafx.h"
#include "Effect_JKO_SUV_SmallExplosion.h"
#include "GameInstance.h"

CEffect_JKO_SUV_SmallExplosion::CEffect_JKO_SUV_SmallExplosion(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_SUV_SmallExplosion::CEffect_JKO_SUV_SmallExplosion(const CEffect_JKO_SUV_SmallExplosion& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_SUV_SmallExplosion::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_SUV_SmallExplosion::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가 
	if (FAILED(__super::NativeConstruct(pArg)))
		return E_FAIL;

	ZeroMemory(&m_JKOSUVSmallExplosionDesc, sizeof(JKOSUVSMALLEXPLOSIONDESC));
	memcpy(&m_JKOSUVSmallExplosionDesc, pArg, sizeof(JKOSUVSMALLEXPLOSIONDESC));

	m_EffectDesc.eLerp_Option = LERT_EaseInOutQuint;

	DEBUG_BREAK(SetValue());
...
_int CEffect_JKO_SUV_SmallExplosion::Tick(_double TimeDelta)
{
	int temp = 0;
	...
	m_EffectDesc.fAlpha += TimeDelta * m_EffectDesc.fSpeed * 2.f;
...
_int CEffect_JKO_SUV_SmallExplosion::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_SUV_SmallExplosion::Render()
{
...
HRESULT CEffect_JKO_SUV_SmallExplosion::SetBillboardMatrix()
{
	...
	XMStoreFloat4x4(&BillboardMatrix,
		...
		XMMatrixTranslation(m_fStartX + m_fValueX * m_fAlpha, m_fStartY + m_fValueY * m_fAlpha , 0.f)
		* WorldMatrix);
...
HRESULT CEffect_JKO_SUV_SmallExplosion::SetValue()
{
	switch (m_JKOSUVSmallExplosionDesc.eType)
	{
	case TYPE_00:
		m_fStartX = 0.1f;
		m_fStartY = 0.1f;
...
CEffect_JKO_SUV_SmallExplosion* CEffect_JKO_SUV_SmallExplosion::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_SUV_SmallExplosion* pInstance = new CEffect_JKO_SUV_SmallExplosion(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_SUV_SmallExplosion");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_SUV_SmallExplosion::Clone(void* pArg)
{
	CEffect_JKO_SUV_SmallExplosion* pInstance = new CEffect_JKO_SUV_SmallExplosion(*this);
...
void CEffect_JKO_SUV_SmallExplosion::Free()
{
	__super::Free();
}
