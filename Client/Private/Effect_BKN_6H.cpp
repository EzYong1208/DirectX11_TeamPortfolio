#include "stdafx.h"
#include "Effect_BKN_6H.h"
#include "GameInstance.h"

CEffect_BKN_6H::CEffect_BKN_6H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BKN_6H::CEffect_BKN_6H(const CEffect_BKN_6H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BKN_6H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BKN_6H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BKN_6H::Tick(_double TimeDelta)
{
	int temp = 0;

	m_fTime += (_float)TimeDelta * m_EffectDesc.fSpeed;
	m_fTimeDelta += (_float)TimeDelta * 25.f;
...
_int CEffect_BKN_6H::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BKN_6H::Render()
{
	...
	XMStoreFloat4x4(&BillboardMatrix,
		XMMatrixRotationRollPitchYaw(XMConvertToRadians(0.f), XMConvertToRadians(0.f), XMConvertToRadians(-10.f - m_fTimeDelta)) *
		XMMatrixTranslation(-0.3f, 0.23f, -0.3f)
		* WorldMatrix);
...
CEffect_BKN_6H* CEffect_BKN_6H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BKN_6H* pInstance = new CEffect_BKN_6H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BKN_6H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BKN_6H::Clone(void* pArg)
{
	CEffect_BKN_6H* pInstance = new CEffect_BKN_6H(*this);
