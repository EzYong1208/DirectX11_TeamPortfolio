#include "stdafx.h"
#include "Effect_BGT_RKM_Ember.h"
#include "GameInstance.h"

CEffect_BGT_RKM_Ember::CEffect_BGT_RKM_Ember(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_RKM_Ember::CEffect_BGT_RKM_Ember(const CEffect_BGT_RKM_Ember& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_RKM_Ember::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_RKM_Ember::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_RKM_Ember::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_RKM_Ember::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_RKM_Ember::Render()
{
...
CEffect_BGT_RKM_Ember* CEffect_BGT_RKM_Ember::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_RKM_Ember* pInstance = new CEffect_BGT_RKM_Ember(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_RKM_Ember");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_RKM_Ember::Clone(void* pArg)
{
	CEffect_BGT_RKM_Ember* pInstance = new CEffect_BGT_RKM_Ember(*this);
...
void CEffect_BGT_RKM_Ember::Free()
{
	__super::Free();
}
