#include "stdafx.h"
#include "Effect_JKO_2D.h"
#include "GameInstance.h"

CEffect_JKO_2D::CEffect_JKO_2D(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_2D::CEffect_JKO_2D(const CEffect_JKO_2D& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_2D::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_2D::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_2D::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_2D::LateTick(_double TimeDelta)
{
...
CEffect_JKO_2D* CEffect_JKO_2D::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_2D* pInstance = new CEffect_JKO_2D(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_2D");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_2D::Clone(void* pArg)
{
	CEffect_JKO_2D* pInstance = new CEffect_JKO_2D(*this);
