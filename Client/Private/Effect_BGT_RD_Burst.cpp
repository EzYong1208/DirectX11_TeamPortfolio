#include "stdafx.h"
#include "Effect_BGT_RD_Burst.h"
#include "GameInstance.h"

CEffect_BGT_RD_Burst::CEffect_BGT_RD_Burst(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_RD_Burst::CEffect_BGT_RD_Burst(const CEffect_BGT_RD_Burst& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_RD_Burst::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_RD_Burst::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_RD_Burst::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_RD_Burst::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_RD_Burst::Render()
{
...
CEffect_BGT_RD_Burst* CEffect_BGT_RD_Burst::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_RD_Burst* pInstance = new CEffect_BGT_RD_Burst(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_RD_Burst");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_RD_Burst::Clone(void* pArg)
{
	CEffect_BGT_RD_Burst* pInstance = new CEffect_BGT_RD_Burst(*this);
...
void CEffect_BGT_RD_Burst::Free()
{
	__super::Free();
}
