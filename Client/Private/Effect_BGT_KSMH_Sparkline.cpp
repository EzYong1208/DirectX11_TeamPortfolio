#include "stdafx.h"
#include "Effect_BGT_KSMH_Sparkline.h"
#include "GameInstance.h"

CEffect_BGT_KSMH_Sparkline::CEffect_BGT_KSMH_Sparkline(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_BillboardValueFix_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_KSMH_Sparkline::CEffect_BGT_KSMH_Sparkline(const CEffect_BGT_KSMH_Sparkline& rhs)
	: CEffect_BillboardValueFix_Effect(rhs)
{
}

HRESULT CEffect_BGT_KSMH_Sparkline::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_KSMH_Sparkline::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_KSMH_Sparkline::Tick(_double TimeDelta)
{
	int temp = 0;
	...
	if (fParentCurAcc >= 45.f)
		temp = ISDEAD;

	return temp;
}

_int CEffect_BGT_KSMH_Sparkline::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_KSMH_Sparkline::Render()
{
...
CEffect_BGT_KSMH_Sparkline* CEffect_BGT_KSMH_Sparkline::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_KSMH_Sparkline* pInstance = new CEffect_BGT_KSMH_Sparkline(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_KSMH_Sparkline");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_KSMH_Sparkline::Clone(void* pArg)
{
	CEffect_BGT_KSMH_Sparkline* pInstance = new CEffect_BGT_KSMH_Sparkline(*this);
...
void CEffect_BGT_KSMH_Sparkline::Free()
{
	__super::Free();
}
