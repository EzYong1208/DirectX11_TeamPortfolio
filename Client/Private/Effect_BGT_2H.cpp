#include "stdafx.h"
#include "Effect_BGT_2H.h"
#include "GameInstance.h"

CEffect_BGT_2H::CEffect_BGT_2H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_2H::CEffect_BGT_2H(const CEffect_BGT_2H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_2H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_2H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_2H::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_2H::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_2H::SetMaterialIndex()
{
	...
	if (m_iCount == 4 && fParentCurAcc >= 45.f)
	{
		++m_iCount;
		m_iMaterialIndex = 4;
	}
	if (m_iCount == 5 && fParentCurAcc >= 50.f)
	{
		++m_iCount;
		m_iMaterialIndex = 5;
	}
	if (m_iCount == 6 && fParentCurAcc >= 55.f)
	{
		++m_iCount;
		m_iMaterialIndex = 6;
	}
...
CEffect_BGT_2H* CEffect_BGT_2H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_2H* pInstance = new CEffect_BGT_2H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_2H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_2H::Clone(void* pArg)
{
	CEffect_BGT_2H* pInstance = new CEffect_BGT_2H(*this);
