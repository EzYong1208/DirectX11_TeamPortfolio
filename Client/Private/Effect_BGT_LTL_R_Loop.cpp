#include "stdafx.h"
#include "Effect_BGT_LTL_R_Loop.h"
#include "GameInstance.h"

CEffect_BGT_LTL_R_Loop::CEffect_BGT_LTL_R_Loop(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_LTL_R_Loop::CEffect_BGT_LTL_R_Loop(const CEffect_BGT_LTL_R_Loop& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_LTL_R_Loop::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_LTL_R_Loop::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_LTL_R_Loop::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_LTL_R_Loop::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_LTL_R_Loop::Render()
{
...
_uint CEffect_BGT_LTL_R_Loop::SetMaterialIndex()
{
	...
	if (m_iCount == 5 && fParentCurAcc >= 55.f)
	{
		++m_iCount;
		iResult = ISDEAD;
	}
...
CEffect_BGT_LTL_R_Loop* CEffect_BGT_LTL_R_Loop::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_LTL_R_Loop* pInstance = new CEffect_BGT_LTL_R_Loop(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_LTL_R_Loop");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_LTL_R_Loop::Clone(void* pArg)
{
	CEffect_BGT_LTL_R_Loop* pInstance = new CEffect_BGT_LTL_R_Loop(*this);
...
void CEffect_BGT_LTL_R_Loop::Free()
{
	__super::Free();
}
