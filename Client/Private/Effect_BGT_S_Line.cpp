#include "stdafx.h"
#include "Effect_BGT_S_Line.h"
#include "GameInstance.h"

CEffect_BGT_S_Line::CEffect_BGT_S_Line(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_S_Line::CEffect_BGT_S_Line(const CEffect_BGT_S_Line& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_S_Line::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_S_Line::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_S_Line::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_S_Line::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_S_Line::Render()
{
...
void CEffect_BGT_S_Line::SetMaterialIndex()
{
...
CEffect_BGT_S_Line* CEffect_BGT_S_Line::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_S_Line* pInstance = new CEffect_BGT_S_Line(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_S_Line");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_S_Line::Clone(void* pArg)
{
	CEffect_BGT_S_Line* pInstance = new CEffect_BGT_S_Line(*this);
