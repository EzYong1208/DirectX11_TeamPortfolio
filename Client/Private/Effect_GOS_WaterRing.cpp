#include "stdafx.h"
#include "Effect_GOS_WaterRing.h"
#include "GameInstance.h"

CEffect_GOS_WaterRing::CEffect_GOS_WaterRing(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_BillboardValueFix_Effect(pDevice, pDeviceContext)
{
}

CEffect_GOS_WaterRing::CEffect_GOS_WaterRing(const CEffect_GOS_WaterRing& rhs)
	: CEffect_BillboardValueFix_Effect(rhs)
{
}

HRESULT CEffect_GOS_WaterRing::NativeConstruct_Prototype()
{
...
HRESULT CEffect_GOS_WaterRing::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_GOS_WaterRing::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_GOS_WaterRing::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_GOS_WaterRing::Render()
{
...
CEffect_GOS_WaterRing* CEffect_GOS_WaterRing::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_GOS_WaterRing* pInstance = new CEffect_GOS_WaterRing(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_GOS_WaterRing");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_GOS_WaterRing::Clone(void* pArg)
{
	CEffect_GOS_WaterRing* pInstance = new CEffect_GOS_WaterRing(*this);
...
void CEffect_GOS_WaterRing::Free()
{
	__super::Free();
}
