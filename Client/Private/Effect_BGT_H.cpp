#include "stdafx.h"
#include "Effect_BGT_H.h"
#include "GameInstance.h"

CEffect_BGT_H::CEffect_BGT_H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_H::CEffect_BGT_H(const CEffect_BGT_H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_H::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_H::LateTick(_double TimeDelta)
{
...
_uint CEffect_BGT_H::SetMaterialIndex()
{
	...
	if (m_iCount == 6 && fParentCurAcc >= 50.f)
	{
		++m_iCount;
		iResult = ISDEAD;
	}
...
CEffect_BGT_H* CEffect_BGT_H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_H* pInstance = new CEffect_BGT_H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_H::Clone(void* pArg)
{
	CEffect_BGT_H* pInstance = new CEffect_BGT_H(*this);
