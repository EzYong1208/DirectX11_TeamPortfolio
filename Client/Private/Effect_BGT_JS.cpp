#include "stdafx.h"
#include "Effect_BGT_JS.h"
#include "GameInstance.h"

CEffect_BGT_JS::CEffect_BGT_JS(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_JS::CEffect_BGT_JS(const CEffect_BGT_JS& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_JS::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_JS::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_JS::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_JS::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_JS::SetMaterialIndex()
{
...
CEffect_BGT_JS* CEffect_BGT_JS::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_JS* pInstance = new CEffect_BGT_JS(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_JS");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_JS::Clone(void* pArg)
{
	CEffect_BGT_JS* pInstance = new CEffect_BGT_JS(*this);
