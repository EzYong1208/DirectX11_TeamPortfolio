#include "stdafx.h"
#include "Effect_MAY_Anchor_Smash_Charging.h"
#include "GameInstance.h"

CEffect_MAY_Anchor_Smash_Charging::CEffect_MAY_Anchor_Smash_Charging(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_MAY_Anchor_Smash_Charging::CEffect_MAY_Anchor_Smash_Charging(const CEffect_MAY_Anchor_Smash_Charging& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_MAY_Anchor_Smash_Charging::NativeConstruct_Prototype()
{
...
HRESULT CEffect_MAY_Anchor_Smash_Charging::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_MAY_Anchor_Smash_Charging::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_MAY_Anchor_Smash_Charging::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_MAY_Anchor_Smash_Charging::Render()
{
...
void CEffect_MAY_Anchor_Smash_Charging::Trigger_Activate(const _double& _fTimeDelta)
{
}

HRESULT CEffect_MAY_Anchor_Smash_Charging::SetUp_Components()
{
	return S_OK;
}

CEffect_MAY_Anchor_Smash_Charging* CEffect_MAY_Anchor_Smash_Charging::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_MAY_Anchor_Smash_Charging* pInstance = new CEffect_MAY_Anchor_Smash_Charging(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_MAY_Anchor_Smash_Charging");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_MAY_Anchor_Smash_Charging::Clone(void* pArg)
{
	CEffect_MAY_Anchor_Smash_Charging* pInstance = new CEffect_MAY_Anchor_Smash_Charging(*this);
...
void CEffect_MAY_Anchor_Smash_Charging::Free()
{
	__super::Free();
}
