#include "stdafx.h"
#include "Effect_BGT_TAS_Line.h"
#include "GameInstance.h"

CEffect_BGT_TAS_Line::CEffect_BGT_TAS_Line(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_TAS_Line::CEffect_BGT_TAS_Line(const CEffect_BGT_TAS_Line& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_TAS_Line::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_TAS_Line::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_TAS_Line::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_TAS_Line::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_TAS_Line::Render()
{
...
CEffect_BGT_TAS_Line* CEffect_BGT_TAS_Line::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_TAS_Line* pInstance = new CEffect_BGT_TAS_Line(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_TAS_Line");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_TAS_Line::Clone(void* pArg)
{
	CEffect_BGT_TAS_Line* pInstance = new CEffect_BGT_TAS_Line(*this);
...
void CEffect_BGT_TAS_Line::Free()
{
	__super::Free();
}
