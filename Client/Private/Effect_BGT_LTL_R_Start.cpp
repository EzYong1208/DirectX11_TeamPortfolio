#include "stdafx.h"
#include "Effect_BGT_LTL_R_Start.h"
#include "GameInstance.h"

CEffect_BGT_LTL_R_Start::CEffect_BGT_LTL_R_Start(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
    : CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_LTL_R_Start::CEffect_BGT_LTL_R_Start(const CEffect_BGT_LTL_R_Start& rhs)
    : CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_LTL_R_Start::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_LTL_R_Start::NativeConstruct(void* pArg)
{
    //	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_LTL_R_Start::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_LTL_R_Start::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_LTL_R_Start::Render()
{
...
_uint CEffect_BGT_LTL_R_Start::SetMaterialIndex()
{
...
CEffect_BGT_LTL_R_Start* CEffect_BGT_LTL_R_Start::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_LTL_R_Start* pInstance = new CEffect_BGT_LTL_R_Start(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_LTL_R_Start");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_LTL_R_Start::Clone(void* pArg)
{
	CEffect_BGT_LTL_R_Start* pInstance = new CEffect_BGT_LTL_R_Start(*this);
...
void CEffect_BGT_LTL_R_Start::Free()
{
	__super::Free();
}
