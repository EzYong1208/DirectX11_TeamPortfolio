#include "stdafx.h"
#include "Effect_BGT_FS.h"
#include "GameInstance.h"

CEffect_BGT_FS::CEffect_BGT_FS(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_FS::CEffect_BGT_FS(const CEffect_BGT_FS& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_FS::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_FS::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_FS::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_FS::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_FS::SetMaterialIndex()
{
...
CEffect_BGT_FS* CEffect_BGT_FS::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_FS* pInstance = new CEffect_BGT_FS(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_FS");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_FS::Clone(void* pArg)
{
	CEffect_BGT_FS* pInstance = new CEffect_BGT_FS(*this);
