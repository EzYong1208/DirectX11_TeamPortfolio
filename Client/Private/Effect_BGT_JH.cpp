#include "stdafx.h"
#include "Effect_BGT_JH.h"
#include "GameInstance.h"

CEffect_BGT_JH::CEffect_BGT_JH(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_JH::CEffect_BGT_JH(const CEffect_BGT_JH& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_JH::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_JH::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_JH::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_JH::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_JH::SetMaterialIndex()
{
...
CEffect_BGT_JH* CEffect_BGT_JH::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_JH* pInstance = new CEffect_BGT_JH(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_JH");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_JH::Clone(void* pArg)
{
	CEffect_BGT_JH* pInstance = new CEffect_BGT_JH(*this);
