#include "stdafx.h"
#include "Effect_BGT_CS.h"
#include "GameInstance.h"

CEffect_BGT_CS::CEffect_BGT_CS(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_CS::CEffect_BGT_CS(const CEffect_BGT_CS& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_CS::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_CS::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_CS::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_CS::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_CS::SetMaterialIndex()
{
...
CEffect_BGT_CS* CEffect_BGT_CS::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_CS* pInstance = new CEffect_BGT_CS(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_CS");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_CS::Clone(void* pArg)
{
	CEffect_BGT_CS* pInstance = new CEffect_BGT_CS(*this);
