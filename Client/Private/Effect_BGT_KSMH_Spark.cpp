#include "stdafx.h"
#include "Effect_BGT_KSMH_Spark.h"
#include "GameInstance.h"

CEffect_BGT_KSMH_Spark::CEffect_BGT_KSMH_Spark(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_BillboardValueFix_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_KSMH_Spark::CEffect_BGT_KSMH_Spark(const CEffect_BGT_KSMH_Spark& rhs)
	: CEffect_BillboardValueFix_Effect(rhs)
{
}

HRESULT CEffect_BGT_KSMH_Spark::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_KSMH_Spark::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_KSMH_Spark::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_KSMH_Spark::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_KSMH_Spark::Render()
{
...
CEffect_BGT_KSMH_Spark* CEffect_BGT_KSMH_Spark::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_KSMH_Spark* pInstance = new CEffect_BGT_KSMH_Spark(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_KSMH_Spark");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_KSMH_Spark::Clone(void* pArg)
{
	CEffect_BGT_KSMH_Spark* pInstance = new CEffect_BGT_KSMH_Spark(*this);
...
void CEffect_BGT_KSMH_Spark::Free()
{
	__super::Free();
}
