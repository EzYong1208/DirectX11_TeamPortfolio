#include "stdafx.h"
#include "Effect_BGT_6P.h"
#include "GameInstance.h"

CEffect_BGT_6P::CEffect_BGT_6P(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_6P::CEffect_BGT_6P(const CEffect_BGT_6P& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_6P::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_6P::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_6P::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_6P::LateTick(_double TimeDelta)
{
...
_uint CEffect_BGT_6P::SetMaterialIndex()
{
...
CEffect_BGT_6P* CEffect_BGT_6P::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_6P* pInstance = new CEffect_BGT_6P(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_6P");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_6P::Clone(void* pArg)
{
	CEffect_BGT_6P* pInstance = new CEffect_BGT_6P(*this);
