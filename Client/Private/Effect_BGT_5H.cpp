#include "stdafx.h"
#include "Effect_BGT_5H.h"
#include "GameInstance.h"

CEffect_BGT_5H::CEffect_BGT_5H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_5H::CEffect_BGT_5H(const CEffect_BGT_5H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_5H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_5H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_5H::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_5H::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_5H::SetMaterialIndex()
{
...
CEffect_BGT_5H* CEffect_BGT_5H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_5H* pInstance = new CEffect_BGT_5H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_5H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_5H::Clone(void* pArg)
{
	CEffect_BGT_5H* pInstance = new CEffect_BGT_5H(*this);
