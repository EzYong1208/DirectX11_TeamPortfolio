#include "stdafx.h"
#include "Effect_BGT_2S.h"
#include "GameInstance.h"

CEffect_BGT_2S::CEffect_BGT_2S(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_2S::CEffect_BGT_2S(const CEffect_BGT_2S& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_2S::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_2S::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_2S::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_2S::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_2S::SetMaterialIndex()
{
...
CEffect_BGT_2S* CEffect_BGT_2S::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_2S* pInstance = new CEffect_BGT_2S(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_2S");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_2S::Clone(void* pArg)
{
	CEffect_BGT_2S* pInstance = new CEffect_BGT_2S(*this);
