#include "stdafx.h"
#include "Effect_JKO_FED_Burst.h"
#include "GameInstance.h"

CEffect_JKO_FED_Burst::CEffect_JKO_FED_Burst(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_FED_Burst::CEffect_JKO_FED_Burst(const CEffect_JKO_FED_Burst& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_FED_Burst::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_FED_Burst::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_FED_Burst::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_FED_Burst::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_FED_Burst::Render()
{
...
CEffect_JKO_FED_Burst* CEffect_JKO_FED_Burst::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_FED_Burst* pInstance = new CEffect_JKO_FED_Burst(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_FED_Burst");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_FED_Burst::Clone(void* pArg)
{
	CEffect_JKO_FED_Burst* pInstance = new CEffect_JKO_FED_Burst(*this);
...
void CEffect_JKO_FED_Burst::Free()
{
	__super::Free();
}
