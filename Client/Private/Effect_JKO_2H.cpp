#include "stdafx.h"
#include "Effect_JKO_2H.h"
#include "GameInstance.h"

CEffect_JKO_2H::CEffect_JKO_2H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_2H::CEffect_JKO_2H(const CEffect_JKO_2H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_2H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_2H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_2H::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_2H::LateTick(_double TimeDelta)
{
...
void CEffect_JKO_2H::SetMaterialIndex()
{
	//56ac0a
	_double fParentCurAcc = m_EffectDesc.m_pParent->Get_ModelSlot()->Get_PlayAcc(MODEL_TAG_DEFAULT_BODY);

	if (m_iCount == 0 && fParentCurAcc >= 25.f)
	{
		++m_iCount;
		m_iMaterialIndex = 0;
	}
	if (m_iCount == 1 && fParentCurAcc >= 31.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 2 && fParentCurAcc >= 36.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 3 && fParentCurAcc >= 41.f)
	{
		++m_iCount;
		m_iMaterialIndex = 3;
	}
...
CEffect_JKO_2H* CEffect_JKO_2H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_2H* pInstance = new CEffect_JKO_2H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_2H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_2H::Clone(void* pArg)
{
	CEffect_JKO_2H* pInstance = new CEffect_JKO_2H(*this);
