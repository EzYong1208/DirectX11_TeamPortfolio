#include "stdafx.h"
#include "Effect_BGT_LTL_L_Loop.h"
#include "GameInstance.h"

CEffect_BGT_LTL_L_Loop::CEffect_BGT_LTL_L_Loop(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_LTL_L_Loop::CEffect_BGT_LTL_L_Loop(const CEffect_BGT_LTL_L_Loop& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_LTL_L_Loop::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_LTL_L_Loop::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_LTL_L_Loop::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_LTL_L_Loop::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_LTL_L_Loop::Render()
{
...
_uint CEffect_BGT_LTL_L_Loop::SetMaterialIndex()
{
	_double fParentCurAcc = m_EffectDesc.m_pParent->Get_ModelSlot()->Get_PlayAcc(MODEL_TAG_DEFAULT_BODY);
	_uint	iResult = 0;

	if (m_iCount == 0 && fParentCurAcc >= 35.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 1 && fParentCurAcc >= 40.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 2 && fParentCurAcc >= 45.f)
	{
		++m_iCount;
		m_iMaterialIndex = 3;
	}
	if (m_iCount == 3 && fParentCurAcc >= 50.f)
	{
		++m_iCount;
		m_iMaterialIndex = 4;
	}
	if (m_iCount == 4 && fParentCurAcc >= 55.f)
	{
		++m_iCount;
		iResult = ISDEAD;
	}
...
CEffect_BGT_LTL_L_Loop* CEffect_BGT_LTL_L_Loop::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_LTL_L_Loop* pInstance = new CEffect_BGT_LTL_L_Loop(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_LTL_L_Loop");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_LTL_L_Loop::Clone(void* pArg)
{
	CEffect_BGT_LTL_L_Loop* pInstance = new CEffect_BGT_LTL_L_Loop(*this);
...
void CEffect_BGT_LTL_L_Loop::Free()
{
	__super::Free();
}
