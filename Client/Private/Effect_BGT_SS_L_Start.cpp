#include "stdafx.h"
#include "Effect_BGT_SS_L_Start.h"
#include "GameInstance.h"

CEffect_BGT_SS_L_Start::CEffect_BGT_SS_L_Start(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_SS_L_Start::CEffect_BGT_SS_L_Start(const CEffect_BGT_SS_L_Start& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_SS_L_Start::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_SS_L_Start::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_SS_L_Start::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_SS_L_Start::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_SS_L_Start::Render()
{
	...
	if (1.f >= m_EffectDesc.fAlpha)
	{
		...
		if (1 == m_iMaterialIndex ||
			2 == m_iMaterialIndex)
			iShaderPass = 19;	//	빨강색
		else
			iShaderPass = SHADER_ORANGE_EFFECT_SPRITE;	//	주황색

		m_pModelCom->Bind_OnShader(m_pShaderCom, m_iMaterialIndex, aiTextureType_DIFFUSE, "g_DiffuseTexture");
		m_pModelCom->Render(m_pShaderCom, iShaderPass, m_iMaterialIndex);
	}

	return S_OK;
}

_uint CEffect_BGT_SS_L_Start::SetMaterialIndex()
{
	_double fParentCurAcc = m_EffectDesc.m_pParent->Get_ModelSlot()->Get_PlayAcc(MODEL_TAG_DEFAULT_BODY);
	_uint	iResult = 0;

	if (m_iCount == 0 && fParentCurAcc >= 0.f)
	{
		++m_iCount;
		m_iMaterialIndex = 0;
	}
	if (m_iCount == 1 && fParentCurAcc >= 5.f)
	{
		++m_iCount;
		m_iMaterialIndex = 1;
	}
	if (m_iCount == 2 && fParentCurAcc >= 10.f)
	{
		++m_iCount;
		m_iMaterialIndex = 2;
	}
	if (m_iCount == 3 && fParentCurAcc >= 15.f)
	{
		++m_iCount;
		m_iMaterialIndex = 3;
	}
	if (m_iCount == 4 && fParentCurAcc >= 20.f)
	{
		++m_iCount;
		iResult = ISDEAD;
	}
...
CEffect_BGT_SS_L_Start* CEffect_BGT_SS_L_Start::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_SS_L_Start* pInstance = new CEffect_BGT_SS_L_Start(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_SS_L_Start");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_SS_L_Start::Clone(void* pArg)
{
	CEffect_BGT_SS_L_Start* pInstance = new CEffect_BGT_SS_L_Start(*this);
...
void CEffect_BGT_SS_L_Start::Free()
{
	__super::Free();
}
