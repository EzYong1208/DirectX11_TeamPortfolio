#include "stdafx.h"
#include "Effect_JKO_SUV_Defend.h"
#include "GameInstance.h"

CEffect_JKO_SUV_Defend::CEffect_JKO_SUV_Defend(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_SUV_Defend::CEffect_JKO_SUV_Defend(const CEffect_JKO_SUV_Defend& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_SUV_Defend::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_SUV_Defend::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_SUV_Defend::Tick(_double TimeDelta)
{
	int temp = 0;

	m_fTime += (_float)TimeDelta * m_EffectDesc.fSpeed;
	m_fTimeDelta += TimeDelta * 0.5f;
...
_int CEffect_JKO_SUV_Defend::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_SUV_Defend::Render()
{
	...
	XMStoreFloat4x4(&BillboardMatrix,
		XMMatrixRotationRollPitchYaw(XMConvertToRadians(0.f), XMConvertToRadians(0.f + (m_fTimeDelta * 180)), XMConvertToRadians(0.f)) *
		XMMatrixTranslation(0.11f, 0.3f, -0.2f)
		* WorldMatrix);
...
CEffect_JKO_SUV_Defend* CEffect_JKO_SUV_Defend::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_SUV_Defend* pInstance = new CEffect_JKO_SUV_Defend(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_SUV_Defend");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_SUV_Defend::Clone(void* pArg)
{
	CEffect_JKO_SUV_Defend* pInstance = new CEffect_JKO_SUV_Defend(*this);
...
void CEffect_JKO_SUV_Defend::Free()
{
	__super::Free();
}
