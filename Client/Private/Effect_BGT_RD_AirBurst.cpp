#include "stdafx.h"
#include "Effect_BGT_RD_AirBurst.h"
#include "GameInstance.h"

CEffect_BGT_RD_AirBurst::CEffect_BGT_RD_AirBurst(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_RD_AirBurst::CEffect_BGT_RD_AirBurst(const CEffect_BGT_RD_AirBurst& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_RD_AirBurst::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_RD_AirBurst::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_RD_AirBurst::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_RD_AirBurst::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_RD_AirBurst::Render()
{
...
CEffect_BGT_RD_AirBurst* CEffect_BGT_RD_AirBurst::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_RD_AirBurst* pInstance = new CEffect_BGT_RD_AirBurst(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_RD_AirBurst");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_RD_AirBurst::Clone(void* pArg)
{
	CEffect_BGT_RD_AirBurst* pInstance = new CEffect_BGT_RD_AirBurst(*this);
...
void CEffect_BGT_RD_AirBurst::Free()
{
	__super::Free();
}
