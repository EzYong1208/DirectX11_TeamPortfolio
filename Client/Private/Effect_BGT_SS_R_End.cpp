#include "stdafx.h"
#include "Effect_BGT_SS_R_End.h"
#include "GameInstance.h"

CEffect_BGT_SS_R_End::CEffect_BGT_SS_R_End(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_SS_R_End::CEffect_BGT_SS_R_End(const CEffect_BGT_SS_R_End& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_SS_R_End::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_SS_R_End::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_SS_R_End::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_SS_R_End::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_SS_R_End::Render()
{
...
_uint CEffect_BGT_SS_R_End::SetMaterialIndex()
{
...
CEffect_BGT_SS_R_End* CEffect_BGT_SS_R_End::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_SS_R_End* pInstance = new CEffect_BGT_SS_R_End(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_SS_R_End");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_SS_R_End::Clone(void* pArg)
{
	CEffect_BGT_SS_R_End* pInstance = new CEffect_BGT_SS_R_End(*this);
...
void CEffect_BGT_SS_R_End::Free()
{
	__super::Free();
}
