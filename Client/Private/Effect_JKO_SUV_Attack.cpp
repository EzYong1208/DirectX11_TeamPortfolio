#include "stdafx.h"
#include "Effect_JKO_SUV_Attack.h"
#include "GameInstance.h"

CEffect_JKO_SUV_Attack::CEffect_JKO_SUV_Attack(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_SUV_Attack::CEffect_JKO_SUV_Attack(const CEffect_JKO_SUV_Attack& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_SUV_Attack::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_SUV_Attack::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_SUV_Attack::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_SUV_Attack::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_SUV_Attack::Render()
{
...
CEffect_JKO_SUV_Attack* CEffect_JKO_SUV_Attack::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_SUV_Attack* pInstance = new CEffect_JKO_SUV_Attack(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_SUV_Attack");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_SUV_Attack::Clone(void* pArg)
{
	CEffect_JKO_SUV_Attack* pInstance = new CEffect_JKO_SUV_Attack(*this);
...
void CEffect_JKO_SUV_Attack::Free()
{
	__super::Free();
}
