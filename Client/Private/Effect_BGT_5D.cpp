#include "stdafx.h"
#include "Effect_BGT_5D.h"
#include "GameInstance.h"

CEffect_BGT_5D::CEffect_BGT_5D(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_5D::CEffect_BGT_5D(const CEffect_BGT_5D& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_5D::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_5D::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_5D::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_5D::LateTick(_double TimeDelta)
{
...
void CEffect_BGT_5D::SetMaterialIndex()
{
...
CEffect_BGT_5D* CEffect_BGT_5D::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_5D* pInstance = new CEffect_BGT_5D(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_5D");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_5D::Clone(void* pArg)
{
	CEffect_BGT_5D* pInstance = new CEffect_BGT_5D(*this);
