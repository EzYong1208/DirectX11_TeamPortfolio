#include "stdafx.h"
#include "Effect_JKO_CheerKira.h"
#include "GameInstance.h"

CEffect_JKO_CheerKira::CEffect_JKO_CheerKira(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_CheerKira::CEffect_JKO_CheerKira(const CEffect_JKO_CheerKira& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_CheerKira::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_CheerKira::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
	if (FAILED(__super::NativeConstruct(pArg)))
		return E_FAIL;

	ZeroMemory(&m_JKOCheerKiraDesc, sizeof(JKOCHEERKIRADESC));
	memcpy(&m_JKOCheerKiraDesc, pArg, sizeof(JKOCHEERKIRADESC));
...
_int CEffect_JKO_CheerKira::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_CheerKira::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_CheerKira::Render()
{
...
void CEffect_JKO_CheerKira::SetMaterialIndex()
{
	switch (m_JKOCheerKiraDesc.eIndex)
	{
	case KIRA_BIG:
		m_iMaterialIndex = 0;
		break;
...
CEffect_JKO_CheerKira* CEffect_JKO_CheerKira::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_CheerKira* pInstance = new CEffect_JKO_CheerKira(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_CheerKira");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_CheerKira::Clone(void* pArg)
{
	CEffect_JKO_CheerKira* pInstance = new CEffect_JKO_CheerKira(*this);
...
void CEffect_JKO_CheerKira::Free()
{
	__super::Free();
}
