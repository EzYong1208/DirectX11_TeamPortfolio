#include "stdafx.h"
#include "Effect_JKO_JS_Strike.h"
#include "GameInstance.h"

CEffect_JKO_JS_Strike::CEffect_JKO_JS_Strike(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_JS_Strike::CEffect_JKO_JS_Strike(const CEffect_JKO_JS_Strike& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_JS_Strike::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_JS_Strike::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_JS_Strike::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_JS_Strike::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_JKO_JS_Strike::Render()
{
...
void CEffect_JKO_JS_Strike::SetMaterialIndex()
{
...
CEffect_JKO_JS_Strike* CEffect_JKO_JS_Strike::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_JS_Strike* pInstance = new CEffect_JKO_JS_Strike(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_JS_Strike");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_JS_Strike::Clone(void* pArg)
{
	CEffect_JKO_JS_Strike* pInstance = new CEffect_JKO_JS_Strike(*this);
...
void CEffect_JKO_JS_Strike::Free()
{
	__super::Free();
}
