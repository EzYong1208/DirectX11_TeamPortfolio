#include "stdafx.h"
#include "Effect_JKO_2S.h"
#include "GameInstance.h"

CEffect_JKO_2S::CEffect_JKO_2S(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_JKO_2S::CEffect_JKO_2S(const CEffect_JKO_2S& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_JKO_2S::NativeConstruct_Prototype()
{
...
HRESULT CEffect_JKO_2S::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_JKO_2S::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_JKO_2S::LateTick(_double TimeDelta)
{
...
void CEffect_JKO_2S::SetMaterialIndex()
{
...
CEffect_JKO_2S* CEffect_JKO_2S::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_JKO_2S* pInstance = new CEffect_JKO_2S(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_JKO_2S");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_JKO_2S::Clone(void* pArg)
{
	CEffect_JKO_2S* pInstance = new CEffect_JKO_2S(*this);
