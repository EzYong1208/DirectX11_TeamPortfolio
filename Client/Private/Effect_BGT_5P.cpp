#include "stdafx.h"
#include "Effect_BGT_5P.h"
#include "GameInstance.h"

CEffect_BGT_5P::CEffect_BGT_5P(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_5P::CEffect_BGT_5P(const CEffect_BGT_5P& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_5P::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_5P::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_5P::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_5P::LateTick(_double TimeDelta)
{
...
_uint CEffect_BGT_5P::SetMaterialIndex()
{
...
CEffect_BGT_5P* CEffect_BGT_5P::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_5P* pInstance = new CEffect_BGT_5P(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_5P");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_5P::Clone(void* pArg)
{
	CEffect_BGT_5P* pInstance = new CEffect_BGT_5P(*this);
