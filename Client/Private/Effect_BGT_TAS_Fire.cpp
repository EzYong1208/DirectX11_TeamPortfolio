#include "stdafx.h"
#include "Effect_BGT_TAS_Fire.h"
#include "GameInstance.h"

CEffect_BGT_TAS_Fire::CEffect_BGT_TAS_Fire(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BGT_TAS_Fire::CEffect_BGT_TAS_Fire(const CEffect_BGT_TAS_Fire& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BGT_TAS_Fire::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BGT_TAS_Fire::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BGT_TAS_Fire::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BGT_TAS_Fire::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BGT_TAS_Fire::Render()
{
...
CEffect_BGT_TAS_Fire* CEffect_BGT_TAS_Fire::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BGT_TAS_Fire* pInstance = new CEffect_BGT_TAS_Fire(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BGT_TAS_Fire");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BGT_TAS_Fire::Clone(void* pArg)
{
	CEffect_BGT_TAS_Fire* pInstance = new CEffect_BGT_TAS_Fire(*this);
...
void CEffect_BGT_TAS_Fire::Free()
{
	__super::Free();
}
