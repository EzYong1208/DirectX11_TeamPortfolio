#include "stdafx.h"
#include "Effect_BKN_26H_H.h"
#include "GameInstance.h"

CEffect_BKN_26H_H::CEffect_BKN_26H_H(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_BKN_26H_H::CEffect_BKN_26H_H(const CEffect_BKN_26H_H& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_BKN_26H_H::NativeConstruct_Prototype()
{
...
HRESULT CEffect_BKN_26H_H::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_BKN_26H_H::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_BKN_26H_H::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_BKN_26H_H::Render()
{
...
CEffect_BKN_26H_H* CEffect_BKN_26H_H::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_BKN_26H_H* pInstance = new CEffect_BKN_26H_H(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_BKN_26H_H");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_BKN_26H_H::Clone(void* pArg)
{
	CEffect_BKN_26H_H* pInstance = new CEffect_BKN_26H_H(*this);
