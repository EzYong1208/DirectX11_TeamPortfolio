...
CEffect_Anim_Effect::CEffect_Anim_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect(pDevice, pDeviceContext)
{
}

CEffect_Anim_Effect::CEffect_Anim_Effect(const CEffect_Anim_Effect& rhs)
	: CEffect(rhs)
{
}

HRESULT CEffect_Anim_Effect::NativeConstruct_Prototype()
{
...
HRESULT CEffect_Anim_Effect::NativeConstruct(void* pArg)
{
	//	1. 게임인스턴스 준비
	...
	{
		ANIMEFFECTDESC temp;
		memcpy(&temp, pArg, sizeof(ANIMEFFECTDESC));
...
_int CEffect_Anim_Effect::Tick(_double TimeDelta)
{
...
_int CEffect_Anim_Effect::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_Anim_Effect::Render()
{

	return S_OK;
}

_bool CEffect_Anim_Effect::Check_Ground(_fvector _vPos)
{
	return true;
}

void CEffect_Anim_Effect::Trigger_Activate(const _double& _fTimeDelta)
{
}

HRESULT CEffect_Anim_Effect::SetUp_Components()
{
	//	1. 셰이더 추가
	/* For.Com_Shader */
	if (FAILED(__super::Add_Component(
...
HRESULT CEffect_Anim_Effect::SetUp_ConstantTable_BillBoarding()
{
...
HRESULT CEffect_Anim_Effect::SetUp_ConstantTable()
{
	return S_OK;
}

CEffect_Anim_Effect* CEffect_Anim_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_Anim_Effect* pInstance = new CEffect_Anim_Effect(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_Anim_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_Anim_Effect::Clone(void* pArg)
{
	CEffect_Anim_Effect* pInstance = new CEffect_Anim_Effect(*this);
