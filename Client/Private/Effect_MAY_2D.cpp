#include "stdafx.h"
#include "Effect_MAY_2D.h"
#include "GameInstance.h"

CEffect_MAY_2D::CEffect_MAY_2D(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CEffect_None_Anim_Effect(pDevice, pDeviceContext)
{
}

CEffect_MAY_2D::CEffect_MAY_2D(const CEffect_MAY_2D& rhs)
	: CEffect_None_Anim_Effect(rhs)
{
}

HRESULT CEffect_MAY_2D::NativeConstruct_Prototype()
{
...
HRESULT CEffect_MAY_2D::NativeConstruct(void* pArg)
{
	//	트랜스폼, 모델, 셰이더, 랜더러 추가
...
_int CEffect_MAY_2D::Tick(_double TimeDelta)
{
	int temp = 0;
...
_int CEffect_MAY_2D::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_MAY_2D::Render()
{
	...
	XMStoreFloat4x4(&BillboardMatrix,
		...
		XMMatrixTranslation(0.25f, -0.25f, 1.3f)
		* WorldMatrix);
...
void CEffect_MAY_2D::Trigger_Activate(const _double& _fTimeDelta)
{
}

HRESULT CEffect_MAY_2D::SetUp_Components()
{
	return S_OK;
}

CEffect_MAY_2D* CEffect_MAY_2D::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CEffect_MAY_2D* pInstance = new CEffect_MAY_2D(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CEffect_MAY_2D");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CEffect_MAY_2D::Clone(void* pArg)
{
	CEffect_MAY_2D* pInstance = new CEffect_MAY_2D(*this);
